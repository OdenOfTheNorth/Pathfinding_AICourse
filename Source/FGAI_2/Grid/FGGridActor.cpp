...
#include "FGLineBatchComponent.h"
...
AFGGridActor::AFGGridActor()
{
	...
	FGLineBatch = CreateDefaultSubobject<UFGLineBatchComponent>(TEXT("FGLineBatchComponent"));
	FGLineBatch->SetupAttachment(RootComponent);
}
...
void AFGGridActor::DrawBlocks()
{
	...
	for (int32 Y = Height - 1; Y >= 0; --Y)
	{
		for (int32 X = 0; X < Width; ++X)
		{
			...
			//FGLineBatch->DrawSolidBox(Box, FTransform::Identity, Color, 0, 0.0f);
		}
	}
}
...
void AFGGridActor::GetNeighborsFromWorldPosition(FVector currentPos, TArray<FFGTileinfo> Neighbors)
{
	TArray<int> tileIndex;
	/*
	tileIndex[0] = GetTileIndexFromWorldLocation(FVector(currentPos.X + TileSize,currentPos.Y,currentPos.Z ));
	tileIndex[1] = GetTileIndexFromWorldLocation(FVector(currentPos.X + TileSize,currentPos.Y - TileSize,currentPos.Z ));
	tileIndex[2] = GetTileIndexFromWorldLocation(FVector(currentPos.X ,currentPos.Y - TileSize,currentPos.Z ));
	tileIndex[3] = GetTileIndexFromWorldLocation(FVector(currentPos.X - TileSize,currentPos.Y - TileSize,currentPos.Z ));
	tileIndex[4] = GetTileIndexFromWorldLocation(FVector(currentPos.X - TileSize,currentPos.Y,currentPos.Z ));
	tileIndex[5] = GetTileIndexFromWorldLocation(FVector(currentPos.X - TileSize,currentPos.Y + TileSize,currentPos.Z ));
	tileIndex[6] = GetTileIndexFromWorldLocation(FVector(currentPos.X ,currentPos.Y + TileSize,currentPos.Z ));
	tileIndex[7] = GetTileIndexFromWorldLocation(FVector(currentPos.X + TileSize,currentPos.Y + TileSize,currentPos.Z ));
*/
	int currentIdex = GetTileIndexFromWorldLocation(currentPos);
	//tileIndex[0] = (currentIdex)
	
	for (int i = 0; i < tileIndex.Num() - 1; i++)
	{
		Neighbors[i] = TileList[tileIndex[i]];
	}
